...
PASTA_SAIDA = "02_autor_hashtag_sem_genericas"

ARQ_NODES = "nodes_author_hashtag.csv"
ARQ_EDGES = "edges_author_hashtag.csv"

SAIDA_IMG = "rede_autor_hashtag.png"
SAIDA_STATS = "estatisticas_autor_hashtag.csv"
...
def main():
    ...
    for c in ["Id", "Type"]:
        if c not in nodes.columns:
            raise ValueError(f"{ARQ_NODES} precisa ter a coluna '{c}'")
    for c in ["Source", "Target", "Weight"]:
        if c not in edges.columns:
            raise ValueError(f"{ARQ_EDGES} precisa ter a coluna '{c}'")
    ...
    for _, r in nodes.iterrows():
        ...
        G.add_node(node_id, node_type=ntype, frequency=freq)
    ...
    for _, r in edges.iterrows():
        ...
        if u not in G:
            G.add_node(u, node_type="author", frequency=0)
        if v not in G:
            G.add_node(v, node_type="hashtag", frequency=0)
    ...
    if COLORIR_POR == "modularity":
        ...
        subtitulo_cor = f"cores=modularity({part_method})"
    else:
        node_colors = [
            (0.12, 0.47, 0.71, 1.0) if G.nodes[n].get("node_type") == "author" else (0.20, 0.63, 0.17, 1.0)
            for n in node_list
        ]
        subtitulo_cor = "cores=tipo(author/hashtag)"
    ...
    plt.title(f"Rede Autor × Hashtag (Sem genéricas)")
    plt.axis("off")
    plt.tight_layout()
    ...
    stats = pd.DataFrame({
        "node": node_list,
        "node_type": [G.nodes[n].get("node_type") for n in node_list],
    ...
    print("[OK] Gerado dentro de 02_gephi_autor_hashtag_sem_genericas:")
    print(" -", out_img)
    print(" -", out_stats)
